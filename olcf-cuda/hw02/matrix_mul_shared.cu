...
__global__ void mmul(const float *A, const float *B, float *C, int ds) {
  ...
  if ((global_col_id < ds) && (global_row_id < ds)) {
    float sum_over_blocks = 0;
    for (int block_id = 0; block_id < ds / threads_per_block; block_id++) {
      // Load data into shared memory
      As[threadIdx.y][threadIdx.x] =
          A[global_row_id * ds + (block_id * threads_per_block + threadIdx.x)];
      Bs[threadIdx.y][threadIdx.x] =
          B[(block_id * threads_per_block + threadIdx.y) * ds + global_col_id];
      ...
      for (int k = 0; k < threads_per_block; k++)
        sum_over_blocks += As[threadIdx.y][k] *
                           Bs[k][threadIdx.x];  // dot product of row and column
      __syncthreads();
    }

    // Write to global memory
    C[global_row_id * ds + global_col_id] = sum_over_blocks;
  }
}
...
int main() {
  ...
  // Launch kernel
  // dim3 variable holds 3 dimensions
