...
__global__ void row_sums(const float *A, float *sums, size_t ds) {
  ...
  int g_row_idx = blockDim.x * blockIdx.x + threadIdx.x;
  if (g_row_idx < ds) {
    float sum = 0.0f;
...
int main() {
  ...
  row_sums<<<grid, block>>>(d_A, d_sums, DSIZE);
  ...
  column_sums<<<grid, block>>>(d_A, d_sums, DSIZE);
  cudaCheckErrors("kernel launch failure");
  // cuda processing sequence step 2 is complete
